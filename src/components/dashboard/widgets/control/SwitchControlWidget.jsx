...
const SwitchControlWidget = ({ widget = {}, onConfigChange }) => {
  ...
  const addToHistory = (switchId, newState, method = "manual") => {
    const switchConfig = switches.find((sw) => sw.id === switchId)
    const entry = {
      id: Date.now(),
      timestamp: new Date(),
      switchId,
    ...
    setActionHistory((prev) => [entry, ...prev.slice(0, 19)])
  }
  ...
  const getTotalPowerConsumption = () => {
    ...
    return activeSwitches * 150 + Math.random() * 50
  }
  ...
  const handleMainSwitchChange = async (event) => {
  ...
  return (
    <Card className="control-widget switch-control-widget" sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <CardContent sx={{ flexGrow: 1, p: 2 }}>
        ...
        <Box my={4}>
          <FormControlLabel
            control={<Switch checked={isOn} onChange={handleMainSwitchChange} size="medium" color="primary" />}
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={4}>
              ...
              <Typography variant="h6" color="primary">
                {getActiveSwitchCount()}/{switches.length}
              </Typography>
            </Grid>
            <Grid item xs={4}>
              <Typography variant="body2" color="text.secondary">
