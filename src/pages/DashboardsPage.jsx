...
import { Card, Table, Button, Space, Typography, Modal, Checkbox, Upload, message, Tag } from "antd"
import {
...
import { fetchDashboards, saveDashboardToBackend, createDashboardEntry } from "../services/dashboardSave"
...
const DashboardsPage = () => {
  ...
  // Fetch dashboards from backend on mount or reload
  const loadDashboards = async () => {
    ...
    try {
      ...
      setDashboards((data || []).map(d => ({
        ...d,
        key: d.id,
        customerNames: (d.assignedCustomers || []).map(id => customerMap[id] || id),
        public: !!d.public,
      })))
    } catch (err) {
        ...
        <Space size={[0, 4]} wrap>
          {customers && customers.length > 0 ? (
            customers.map((customer, index) => (
              <Tag key={index} color="blue">{customer}</Tag>
            ))
          ) : (
  ...
  const showDeleteConfirm = (dashboard) => {
    Modal.confirm({
      ...
      onOk() {
        ...
        // TODO: Call backend delete if needed
      },
    })
  }
  ...
  //   try {
  //     // Construct a minimal dashboard state for new dashboard
  ...
  //     }
  //     // Save to backend (dashboardId not passed)
  //     const resp = await saveDashboardToBackend({ dashboardJson: JSON.stringify(dashboardState) })
  //     // Add to UI
  //     await loadDashboards()
  //     setIsAddModalVisible(false)
  //     navigate(`/dashboards/${resp.dashboardId}`)
  //   } catch (error) {
  ...
  const handleAddDashboard = async (values) => {
    try {
      ...
      const data = await createDashboardEntry(dashboardDto)
      message.success("Dashboard created!")
      setIsAddModalVisible(false)
      await loadDashboards()
      // Navigate to dashboard editor, use new dashboardId from backend!
      navigate(`/dashboards/${data.dashboardId}`)
    } catch (error) {
      message.error("Failed to create dashboard.")
    }
  }
  ...
  const handleImportDashboard = (file) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Save imported dashboard to backend
        const resp = await saveDashboardToBackend({ dashboardJson: JSON.stringify(dashboard) })
        await loadDashboards()
  ...
  const getFilteredDashboards = () => {
    if (activeTab === "all") return dashboards
    if (activeTab === "assigned") return dashboards.filter((d) => d.assignedCustomers && d.assignedCustomers.length > 0)
    if (activeTab === "unassigned") return dashboards.filter((d) => !d.assignedCustomers || d.assignedCustomers.length === 0)
    return dashboards
  }
